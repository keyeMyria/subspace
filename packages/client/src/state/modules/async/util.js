...
export function toAsync(actionType) {
  return Object.values(AsyncSuffix).map(
    asyncPrefix => `${actionType}${asyncPrefix}`,
  )
}

export function trimSuffix(actionType) {
  return actionType.slice(0, -1)
}

export function getAsyncStateFromActionType(actionType) {
